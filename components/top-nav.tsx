    ...
    <header className="sticky top-0 z-50 border-b border-ink-border bg-[var(--color-surface)]/95 backdrop-blur dark:bg-[var(--color-surface)]/95">
      <div className="mx-auto flex w-full max-w-[1440px] items-center gap-4 px-4 py-3 md:px-6">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`relative rounded-md px-3 py-2 text-sm font-semibold transition-colors ${active ? "text-ink-blue" : "text-ink/70 hover:text-ink dark:text-slate-300 dark:hover:text-white"
                  }`}
              >
                {item.label}
        ...
        <div className="ml-auto flex items-center gap-3">
          <label className="hidden items-center gap-2 rounded-lg border border-ink-border bg-white px-3 py-2 text-sm text-ink/60 shadow-sm md:flex dark:bg-slate-900 dark:text-slate-300">
            <Search className="h-4 w-4" />
            <input
              aria-label="Search"
              placeholder="Search..."
              className="w-48 bg-transparent text-ink outline-none placeholder:text-ink/40 dark:text-slate-100 dark:placeholder:text-slate-400"
            />
          </label>
            ...
            <Link href="/settings">
              <div className="hidden cursor-pointer items-center gap-3 rounded-xl border border-ink-border bg-white px-3 py-2 shadow-sm transition hover:border-ink-blue/40 sm:flex dark:bg-slate-900">
                ...
                <div className="leading-tight">
                  ...
                  <p className="text-[11px] uppercase tracking-[0.08em] text-ink/55 dark:text-slate-400">{session.role}</p>
                </div>
                <ChevronDown className="h-4 w-4 text-ink/45 dark:text-slate-400" />
              </div>
            </Link>
          ) : null}
      ...
      <nav className="flex gap-1 overflow-x-auto border-t border-ink-border/80 px-3 py-2 lg:hidden dark:bg-[var(--color-surface)]">
        {navItems.map((item) => {
          const active = getActive(pathname, item.href);
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`whitespace-nowrap rounded-md px-3 py-1.5 text-xs font-semibold ${active ? "bg-ink-blue text-white" : "bg-white text-ink/70 dark:bg-slate-900 dark:text-slate-300"
                }`}
            >
              {item.label}
            </Link>
